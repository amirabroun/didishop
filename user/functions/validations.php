...
function validatorByRules($rules, $input)
{
    ...
    foreach ($rules as $rule) {
        ...
        if ($rule === 'password') {
            if (isset($_REQUEST[$input]) && strlen($_REQUEST[$input]) < 8) {
                $errors[] = ['rule' => $rule];
            }
        }
        if ($rule === 'password_confirm') {
            if (isset($_REQUEST[$input]) && $_REQUEST[$input] !== POST('password')) {
                $errors[] = ['rule' => $rule];
            }
        }
        if ($rule === 'term') {
...
function translate($word, $is_rule = false)
{
    $attributes = [
        'rules' => [
            ...
            'password_confirm' => 'تکرار کلمه عبور باید با کلمه عبور مطابقت داشته باشد!',
            'mobile' => 'شماره تلفن وارد شده نامعتبر است.',
            'number' => 'مقدار فیلد باید فقط عدد باشد.',
            'term' => 'برای استفاده از سایت, باید قوانین و شرایط سایت دیدیشاپ را بپذیرید!',
